...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'HELP'
Usage: ./server/run_demo.sh [options]
...
CHECK
if $POSTGRES; then
  python3 -c 'import psycopg' || { echo 'Install: pip install -r server/requirements.txt' >&2; exit 1; }
  ./server/postgres.sh up
  DB="$(python3 -m server.postgres_env url --file "${CP_POSTGRES_ENV:-$PWD/server/.env.postgres}")"
fi
if $MQTT; then
  python3 -c 'import paho.mqtt.client' || { echo 'Install: pip install -r server/requirements.txt' >&2; exit 1; }
