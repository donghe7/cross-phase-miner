...
const modelLabel = (status) =>
  ({ unlearned: '未建模', learning: '学习中', reliable: '可预测', unavailable: '数据待更新' })[
    status
  ] || '未建模';
...
async function boot() {
  try {
    ...
    $('all-summary').textContent =
      `全部 ${state.metas.length.toLocaleString()} 个路口 · 展开全城概览`;
    buildMap();
    $('error').hidden = true;
    connect();
    if (!state.started) {
...
function applySnapshot(snapshot) {
  ...
  if (!state.speedPending && !state.speedError)
    $('speed-status').textContent = `当前 ${snapshot.speed}× · 全局生效`;
  paintSpeed();
  if (!state.selected) {
...
async function refreshDetail() {
  ...
  try {
    ...
    if (state.selected === id) {
      ...
      if (!state.detail)
        $('detail').innerHTML = '<p class="empty">路口详情暂时无法读取，正在重试。</p>';
    }
  } finally {
...
function paint() {
  ...
  $('clock-meta').textContent =
    `${snapshot.period === 'day' ? '日间' : '夜间'} / KST · ${snapshot.speed}× 仿真`;
  ...
  $('robot-summary').textContent =
    `${robots.length - travel - stale} 台在路口 · ${travel} 台行驶中 · ${stale} 台待更新`;
  const counts = snapshot.counts;
  $('observed-count').textContent = counts.observed_red + counts.observed_green;
  $('coverage-summary').textContent =
    `${counts.predicted_red + counts.predicted_green} 处模型预测 · ${counts.disputed} 处共识不足`;
  ...
  const metrics = [
    ['数据库', state.config.database_backend === 'postgresql' ? 'PostgreSQL' : 'SQLite'],
    ['观测上传', state.config.observation_transport === 'mqtt' ? 'MQTT' : 'HTTP'],
    ['上传帧数', stats.observations.toLocaleString()],
    ['低于门槛丢弃', stats.rejected.toLocaleString()],
    ['可靠 / 已学模型', `${stats.models_reliable} / ${stats.models}`],
    ['摄取速度', `${stats.obs_per_sec}/s`],
    ['学习队列', stats.queue_depth],
  ];
  setHTML(
    $('ledger'),
...
function paintTracking(robots) {
  ...
  if (!robot) {
    setHTML(
      $('tracking'),
      `<p class="empty">${robots.length ? '点击下方机器人编号，跟随它的当前位置与观测。当前可自由查看路口。' : '暂无机器人上报。启动 fleet 后，会显示当前位置和行驶目的地。'}</p>`,
    );
    return;
  }
  ...
  const note = robotStale(robot)
    ? '上报已过期，实际位置待确认'
    : travelling
      ? `${eta > 0 ? `预计 ${Math.ceil(eta)} 仿真秒后到达` : '等待到达上报'} · 行程进度估算，无 GPS 定位`
      : `${robotAction(robot)} · ${age(robot.updated_at).toFixed(1)} 仿真秒前上报`;
  if (!$('track-content')) {
    $('tracking').innerHTML =
      '<div id="track-content" class="track-content"></div><button id="stop-follow" type="button">取消跟随</button>';
    $('stop-follow').onclick = () => {
      state.followed = null;
      paint();
    };
  ...
  setHTML(
    $('track-content'),
    `<span class="robot-icon">${esc(shortId(robot.robot_id))}</span><div class="track-info"><span class="eyebrow">正在跟随${travelling ? ' · 行驶目的地' : ' · 当前路口'}</span><strong title="${esc(location)}">${esc(location)}</strong><p title="${esc(note)}">${esc(note)}</p><div class="track-progress" style="visibility:${travelling ? 'visible' : 'hidden'}"><i style="width:${progress}%"></i></div></div>`,
  );
}
function paintFleet(robots) {
  ...
  $('fleet-page').textContent = robots.length
    ? `第 ${state.fleetPage + 1} / ${pages} 页 · 显示 ${state.fleetPage * 5 + 1}–${Math.min(robots.length, state.fleetPage * 5 + 5)} / ${robots.length} 台`
    : '每页 5 台 · 暂无机器人';
  ...
  robots.forEach((robot) => {
    ...
    if (!row) {
      ...
      button.setAttribute('aria-label', `跟随 ${robot.robot_id}`);
    ...
    setHTML(
      row.children[3],
      obs
        ? `<span class="${obs.color === 'RED' ? 'red-text' : obs.color === 'GREEN' ? 'green-text' : 'muted'}">${colorName(obs.color)}</span><small>${observationStale(obs) ? '已过期 · ' : ''}${age(obs.timestamp).toFixed(1)} 仿真秒前</small>`
        : '<span class="muted">—</span><small>暂无观测</small>',
    );
    setHTML(
      row.children[4],
      obs
        ? `<strong>${percent(obs.confidence)}</strong> / ${percent(state.config.obs_confidence_threshold)}<small class="${obs.accepted ? '' : 'red-text'}">${obs.accepted ? '该帧已接收' : '低于门槛 · 已丢弃'}</small>`
...
function paintMap(robots) {
  ...
  state.metas.forEach((meta, index) => {
    ...
    const tags = present
      .map(
        (r) =>
          `<span class="badge ${robotStale(r) ? 'amber' : 'blue'}">${esc(shortId(r.robot_id))}${robotStale(r) ? ' 待更新' : ' 在此'}</span>`,
      )
      .concat(
        approaching.map(
          (r) =>
            `<span class="badge ${robotStale(r) ? 'amber' : ''}">→ ${esc(shortId(r.robot_id))} ${robotStale(r) ? '待确认' : '前往'}</span>`,
        ),
      );
    ...
    let modelText =
      `<span class="card-model"><span>模型置信度 <strong>${percent(summary.confidence)}</strong></span><span class="badge ${summary.status === 'reliable' ? 'green' : ''}">${modelLabel(summary.status)}</span></span>` +
    ...
    if (!state.snapshot.learning)
      modelText =
        '<span class="card-progress">学习详情待更新</span><span class="card-evidence">重启后端后可查看进度与到访记录</span>';
    setHTML(
      card,
      `<span class="crossing-top"><span><i class="dot ${colors[code]}"></i>${label[code]}</span>${meta.is_hub ? '<span>枢纽</span>' : ''}</span><span class="name" title="${esc(meta.name)}">${esc(meta.name)}</span>${modelText}<span class="occupants">${tags.join('') || '<span class="vacant">暂无机器人驻留</span>'}</span>`,
    );
    card.setAttribute(
      'aria-label',
      `${tile.title}，${present.map((r) => shortId(r.robot_id)).join('、') || '无机器人驻留'}`,
    );
  });
...
function signalComparison(detail, stale, drift) {
  ...
  const observationNote = stale
    ...
    : live.state === 'disputed'
      ? '机器人观测存在分歧'
      : observed
        ? `${live.voters} 台有效投票 · 快照内 ${Math.max(0, live.age).toFixed(1)} 秒前`
        : '等待机器人有效观测';
  ...
  const predictionNote = stale
    ...
    : expired
      ...
      : predictedColor && remaining != null
        ? `${predictedColor === 'GREEN' ? '绿灯剩余' : '距绿灯'}约 ${Math.max(0, remaining - drift).toFixed(0)} 秒`
        : detail.model?.reliable
          ? '重启后端后刷新页面'
          : '等待学习达标';
...
function paintDetail() {
  ...
  const consensusNote = !live.voters
    ? '没有有效投票，暂不判断共识。'
    : `${live.voters} 台机器人有效投票 · ${live.agreement + 1e-9 >= config.consensus_threshold ? '达到共识门槛' : '低于门槛，实时灯色不采纳'}`;
  const model = detail.model;
  const learning = detail.learning || summaryOf(detail.intersection_id, model);
  setHTML(
    $('detail'),
    `<h3 class="detail-title">${esc(detail.meta.name)}</h3><p class="detail-sub">${esc(detail.intersection_id)} · ${esc(detail.meta.district)}${detail.meta.in_zone ? ' · 配送区' : ''}</p>
    ...
    ${
      observers.length
        ? observers
            ...
            .join('')
        : '<p class="empty">暂无驻留机器人上传的观测，当前置信度为 —。</p>'
    }
    ...
    <section class="detail-section"><h3>周期学习 <span class="badge ${model?.reliable ? 'green' : ''}">${model?.reliable ? '可预测' : '学习中'}</span></h3>${
      model
        ? `<table class="kv"><thead><tr><th>时长</th><th>学习值</th><th>仿真真值</th></tr></thead><tbody>${[
            ['总周期', 'T_cycle'],
            ['红灯', 'T_red'],
            ['绿灯', 'T_green'],
          ]
            ...
            .join(
              '',
            )}</tbody></table><p class="detail-note">${model.samples} 次跳变样本 · 真值仅用于仿真对照。</p>`
        : '<p class="empty">尚无周期模型。机器人将通过侦察采样学习信号周期。</p>'
    }<p class="detail-note">此路口累计接收 ${live.accepted.toLocaleString()} 帧，丢弃 ${live.rejected.toLocaleString()} 帧。</p></section>`,
  );
}
...
function learningPanel(summary) {
  ...
  if (progressOf(summary).estimate_basis === 'unavailable')
    return `<section class="detail-section learning-panel"><h3>模型学习进度</h3><div class="model-score"><strong>${percent(summary.confidence)}</strong><span>${modelLabel(summary.status)}</span></div><p class="detail-note">后端尚未提供学习进度数据，请重启后端后刷新页面。</p></section>`;
  const history = summary.history || {};
  const milestone = (title, event, fallback) =>
    `<li><span>${title}</span><strong>${event ? simTime(event.sim_time) + ' 仿真' : fallback}</strong>${event ? `<small>${esc(new Date(event.wall_time * 1000).toLocaleString('zh-CN', { timeZone: 'Asia/Seoul', hour12: false }))} KST 记录 · ${event.samples} 次跳变 · ${percent(event.confidence)}<br>触发更新：${event.robots.map(shortId).map(esc).join('、') || '—'}</small>` : ''}</li>`;
  const progress = progressOf(summary);
  const basis = {
    ready: '当前时段模型已可用于预测，后续观测仍会更新模型。',
    ...
    planning: '暂无侦察产出历史，按仿真中一次完整侦察约采到 3–4 次跳变估算。',
