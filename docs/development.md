...
| `make test-delivery` | 临时后端验证真实 HTTP 确认丢失、双方重启、补传和去重学习 |
| `make test-mqtt` | 连接既有 broker，在唯一 topic 前缀下验证上报、确认丢失后的补传、重复到访、订阅查询和 SQLite / PostgreSQL 持久化 |
| `make benchmark` | 在临时 SQLite 中测量状态内存、摄取吞吐与学习开销 |
| `./experiments/run.sh v1 test` | 当前离线核心测试 |
| `./experiments/run.sh v2 test` | 从归档解压并隔离执行历史 v2 测试 |
